...
import {keywordsData,negativeKeywordsData,urlInclusionsData ,chartsDataPoints} from '@infrastructure/mock-data'
export const KeywordsPage: React.FC = () => {
  ...
  const getPolylinePoints = () => {
    return chartsDataPoints.map(p => `${p.x},${p.y}`).join(' ');
  };

  // Render chart polygon points for area fill
  const getPolygonPoints = () => {
    const points = chartsDataPoints.map(p => `${p.x},${p.y}`).join(' ');
          ...
          <div className="p-6 bg-gray-50">
            <div className="h-64 bg-white border border-gray-200 rounded relative">
              <svg className="w-full h-full" viewBox="0 0 1400 256" preserveAspectRatio="none">
                ...
                {/* Data points */}
                {chartsDataPoints.map((point, idx) => (
